  ...
  return (
    <SafeAreaView style={[s.safe, { backgroundColor: theme.pageBg }]}>
      ...
      <ScrollView contentContainerStyle={s.scroll}>
        {/* Avatar card */}
        <View style={[s.avatarCard, { backgroundColor: '#061a14' }]}>
          ...
          <View style={s.section}>
            <View style={s.statsGrid}>
              {[
                ...
                { label: 'Streak 🔥',  value: stats.streak,    color: '#f5a623' },
                { label: 'Present',    value: stats.present,   color: '#00d4aa' },
                { label: 'Submitted',  value: stats.submitted, color: '#7c6af5' },
              ].map((st, i) => (
            ...
            <View style={s.badgesGrid}>
              {BADGES.map(badge => {
                ...
                return (
                  <TouchableOpacity
                    key={badge.id}
                    style={[s.badgeCard, { backgroundColor: theme.cardBg, borderColor: earned ? badge.color : theme.border, opacity: earned ? 1 : 0.45 }]}
                  ...
                  >
                    ...
                    {earned && (
                      <View style={{ backgroundColor: badge.color + '22', borderRadius: 10, paddingHorizontal: 8, paddingVertical: 2, borderWidth: 1, borderColor: badge.color + '44' }}>
        ...
        {activeTab === 'settings' && (
          <View style={s.section}>
            ...
            <TouchableOpacity style={[s.logoutBtn, { borderColor: '#ef444466' }]} onPress={handleLogout}>
              <Text style={[s.logoutText, { color: '#ef4444' }]}>🚪 Logout</Text>
            </TouchableOpacity>
          </View>
        )}
      ...
      <Modal visible={!!selectedBadge} transparent animationType="fade" onRequestClose={() => setSelectedBadge(null)}>
        <TouchableOpacity style={{ flex:1, backgroundColor:'#000000aa', alignItems:'center', justifyContent:'center' }} onPress={() => setSelectedBadge(null)} activeOpacity={1}>
          {selectedBadge && (
            <View style={{ backgroundColor: theme.cardBg, borderRadius:20, padding:28, alignItems:'center', gap:12, marginHorizontal:32, borderWidth:2, borderColor: selectedBadge.earned ? selectedBadge.color : theme.border }}>
              <Text style={{ fontSize:56 }}>{selectedBadge.icon}</Text>
              <Text style={{ fontSize:20, fontWeight:'800', color: selectedBadge.earned ? selectedBadge.color : theme.textMuted }}>{selectedBadge.label}</Text>
              <Text style={{ fontSize:14, color: theme.textMuted, textAlign:'center', lineHeight:20 }}>{selectedBadge.desc}</Text>
              {selectedBadge.earned ? (
                <View style={{ backgroundColor: selectedBadge.color + '22', borderRadius:12, paddingHorizontal:16, paddingVertical:8, borderWidth:1, borderColor: selectedBadge.color + '44' }}>
                  <Text style={{ color: selectedBadge.color, fontWeight:'700', fontSize:14 }}>🎉 You earned this badge!</Text>
                </View>
              ) : (
                <View style={{ backgroundColor:'#33333344', borderRadius:12, paddingHorizontal:16, paddingVertical:8 }}>
                  <Text style={{ color: theme.textMuted, fontSize:13 }}>🔒 Not earned yet — keep going!</Text>
                </View>
              )}
              <TouchableOpacity style={{ marginTop:4, paddingHorizontal:24, paddingVertical:10, borderRadius:10, backgroundColor: theme.border }} onPress={() => setSelectedBadge(null)}>
...
const s = StyleSheet.create({
  ...
  avatarCard: { alignItems: 'center', padding: 28 },
