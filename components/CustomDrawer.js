...
// Replaces: web Sidebar component
// Shows role-based nav items + user info + logout
...
// ── Nav items per role (matches web Sidebar NAV_ITEMS) ─────────────────────
const NAV_ITEMS = {
  student: [
  ...
  admin: [
    { label: 'Dashboard',  screen: 'Dashboard', icon: '👑' },
    { label: 'Students',   screen: 'Students',  icon: '👥' },
    { label: 'Courses',    screen: 'Courses',   icon: '📚' },
    { label: 'Analytics',  screen: 'Analytics', icon: '📊' },
...
export default function CustomDrawer({ role = 'student', state, navigation: drawerNav, ...rest }) {
  ...
  const accentColor = ROLE_ACCENT[role] || '#00d4aa';
  const items       = NAV_ITEMS[role] || NAV_ITEMS.student;

  // Active screen name from drawer state
  const activeRoute = state?.routes[state.index]?.name || '';

  const handleNav = (item) => {
    drawerNav.closeDrawer();
    if (item.tab) {
      // Navigate to the tab (inside MainTabs)
        ...
        {items.map((item, i) => {
          ...
          return (
            ...
            >
              ...
              <Text style={[
                s.navLabel,
                { color: isActive ? accentColor : theme.navInactiveColor },
      ...
      <View style={[s.footer, { borderTopColor: theme.border }]}>
        ...
        <View style={s.userRow}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={[s.userEmail, { color: theme.textMuted }]} numberOfLines={1}>
              {user?.email || ''}
            </Text>
          </View>
